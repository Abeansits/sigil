...
## Identity bootstrapping (prerequisite, out of routing scope)

The Migration section's `sigil session launch -t vigil-slack …` step quietly assumes a `vigil-slack` conductor identity already exists — `SOUL.md`, `OPS.md`, `CLAUDE.md`, `state.json` — analogous to today's `~/.agent-deck/conductor/slack-ops/` directory. Routing has no opinion on this, but the cutover doesn't actually work end-to-end without it. Calling it out explicitly so it doesn't get rediscovered during PR B.

1. **What the routing change does NOT do.** `BridgePlatformConfig.default_session` is a `String` that the bridge runner resolves to a `SessionId` against the live session pool at startup. The fail-closed check verifies *pool membership only* — that some session in `sigil-store` has `title = "vigil-slack"`. It does **not** verify that the session's working directory contains identity files, that the agent loaded them, or that anything is actually responding.
2. **Why it matters.** Without identity files, `session launch -t vigil-slack -c claude` produces a fresh Claude Code session with no character, no operating rules, no episodic memory. The first DM that lands via the new routing path goes to a vanilla agent. That agent will respond — but it has no idea what `vigil-slack` is supposed to mean, no `OPS.md` discipline, no continuity with prior conversations.
3. **Recommended path for the cutover.** Port the existing `~/.agent-deck/conductor/slack-ops/` and `~/.agent-deck/conductor/telegram-ops/` directories into a `~/.vigil-vault/` (or `~/.sigil/conductors/`) equivalent during the PR B cutover window. Preserve the `SOUL.md` / `OPS.md` / `CLAUDE.md` / `state.json` structure. The actual content can stay close to today's per-channel ops files — only the labels need to change from `slack-ops` to `vigil-slack` (and the equivalent for telegram). The identity-reload mechanism (`docs/design/identity-reload.md`) already handles re-loading those files on `PostCompact` / `Restart` once an `IdentitySpec` is configured for the session.
4. **Open question — does config implicitly require identity?** Two reasonable answers, with different operator-experience tradeoffs:
   - **No (current proposal):** routing only checks pool membership; identity-file presence is the operator's responsibility. Pro: routing stays narrow and KISS. Con: a misconfigured cutover ships a useless agent and the only signal is degraded conversation quality, not a fail-closed startup error.
   - **Yes (alternative):** extend the startup resolution to also confirm the destination session has an `IdentitySpec` configured (`SessionRecord.identity.is_some()`) and warn / refuse if not. Pro: fail-loud at the right time. Con: couples routing to identity, and `IdentitySpec` is itself optional — operators may legitimately want a destination session without identity files (a fresh sandbox, an experiment).

   Recommendation: stay with **No** for PR B; surface the prerequisite in operator docs and in the PR B description's "before you cut over" checklist. Revisit if the misconfigured-launch failure mode actually bites.
5. **PR B implementation brief — explicit step.** Before the config-add step in §Migration, add: *"Create the conductor identity directory (port from `~/.agent-deck/conductor/<surface>-ops/` if available, or scaffold fresh `SOUL.md` / `OPS.md` / `CLAUDE.md` / `state.json`). Reference it via `--identity SOUL.md,OPS.md,state.json` on `sigil session launch`, or via `[identity]` in the project's `.sigil/config.toml`."* This step is operator-side, not code; it lives in the PR B description and in `docs/USE-CASES.md`'s bridge-cutover walkthrough.
