...
- **Fail-closed guard for declared plain text:** Before stripping plain-text content, run a quick heuristic scan for markup-like patterns. If the body contains strong structural markers of HTML (`<` followed by tag-like tokens, `<!-- -->` patterns) or Markdown (`###` at line start in suspicious density, `[[` wiki-link style), treat it as a polyglot attack and **reject** with `Severity::High` in `SanitizeReport` rather than processing. This closes the gap in threat model #8 where a server lies about `Content-Type` to bypass HTML stripping entirely.
