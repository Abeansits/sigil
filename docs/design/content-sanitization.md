...
8. **Polyglot / mismatched content-type.** A server claims `text/plain` but serves HTML with `<script>` and hidden tags. The agent's "just read the text" path doesn't strip tags because it wasn't told it was HTML. **Mitigated by fail-closed guard (Stage 3):** Declared format is authoritative for *routing*, but plain-text content containing strong structural markers of HTML/Markdown is detected and quarantined before pattern scanning. The guard checks for: `<` followed by tag-like tokens, `<!-- -->` comment patterns, Markdown heading/list syntax in suspicious positions. Content that triggers the guard is rejected with `Severity::High` in `SanitizeReport` rather than silently processed. This closes the gap where a lying server bypasses HTML stripping entirely.
...
**Pre-dispatch content-type reroute (PR3.6).** Before a sanitizer is selected, `dispatch_sanitize` runs an ASCII-only byte sniff on the raw payload. When the caller declared `ContentType::PlainText` and the leading bytes contain a document-root HTML marker (`<!DOCTYPE html>` or `<html>`, case-insensitive, within a bounded prefix window), the router reroutes to the HTML sanitizer. The resulting `SanitizeReport` carries `content_type: Html` (the effective path that actually ran) and `routed_from: Some(PlainText)` (the declared type) so policy and audit consumers can see both.

- **Log is deliberately excluded from the reroute.** Terminal captures legitimately contain HTML markers as part of the captured output, and routing those to the HTML sanitizer would destroy the payload the log was meant to preserve.
- **Fragment heuristics stay non-routing.** A body-only HTML fragment (no `<!DOCTYPE` or `<html>` opener) matches `FMT-001` Path B (multiple distinct structural openers + close-tag density) but is **not** rerouted — Path B's FP rate on legitimate security-prose fixtures is too high for a blunt reroute. Path B remains audit-only signal.
- **Wrap-header `content_type:` reflects the effective type.** The HTML sanitizer renders `content_type: text/html` into the wrap header when the reroute fires, so downstream guardrails that read the wrap header never see the lied-about declared type.
- **`SanitizationRequirement::Required(ContentType)` asserts on the effective type.** `sigil-policy::evaluate_result` compares `report.content_type` against the requirement. When a reroute fires, that comparison uses `Html` — the path that actually ran — not the declared `PlainText`. An action that declared `Required(PlainText)` and received a body that rerouted to `Html` will therefore deny, which is the correct behavior: the action promised to fetch plain text and received active content.

This replaces the Phase 1 draft's "reject plain-text-with-markup as `Severity::High`" design: rerouting to the HTML sanitizer is strictly more defensive than rejecting (the payload is actually stripped instead of passed through with a finding), and it removes a forcing function — a legitimate plain-text document that happens to open with a code-block fencing a DOCTYPE can still be sanitized instead of hard-denied.
...
- **`FMT-001` Content-Type mismatch** — declared `PlainText` or `Log` but the body contains HTML structural markers. **Reframed in PR3.6 as `Info`-level audit metadata** (see the *Pre-dispatch content-type reroute* subsection below): the real attacker case — a server lying about `text/plain` while serving `<!DOCTYPE html>` — is now handled by an ASCII pre-dispatch sniff in `dispatch_sanitize` that reroutes to the HTML sanitizer for a safer strip. The pattern-scan emission remains as audit-only metadata for fragment-shape payloads (Path B: ≥ 2 distinct structural openers AND ≥ N close-tag occurrences) that the reroute deliberately does **not** trigger on, because Path B's FP rate on legitimate security-prose fixtures is too high for a blunt reroute. `FMT-001` at `Info` severity contributes zero weight to the risk score.
