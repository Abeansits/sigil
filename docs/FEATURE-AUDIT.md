...
| External-content sanitization | Implemented | `sigil-content` Phase 1 pipeline — 5-stage plain / HTML / markdown / JSON sanitization with nonce-delimited provenance wrap, keyed-HMAC fingerprints, and injection-pattern scan. `SanitizeReport` flows through `ActionResult`; the conductor enforces `SanitizationRequirement` via `ActionService::dispatch_fetch_external_content` (PRs #43, #45, #50, #52–#58) |
