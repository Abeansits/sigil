...
async fn session_create_produces_valid_audit_entry() {
    ...
    ops_audit::chain::verify_chain(&key, &[entry]).expect("chain should be valid");
}
...
async fn multiple_audit_events_form_valid_chain() {
    ...
    ops_audit::chain::verify_chain(&key, &entries).expect("chain should be valid");
...
async fn init_audit_writer_creates_file_in_data_dir() {
    ...
    let writer = ops_cli::audit::init_audit_writer(dir.path())
        .await
        .expect("init should succeed");

    // Write a test event to confirm it works end-to-end.
    ops_cli::audit::log_event(&writer, "test.init", "test", PolicyDecision::Allow, None).await;
